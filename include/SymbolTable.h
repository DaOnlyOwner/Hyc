...
public:
	...
	bool add(MetaType&& mt) { return m_meta_types.Add(mt.get_name(), std::move(mt)); }
	...
	std::pair<std::vector<Function>*,size_t> get_funcs(const std::string& name) { auto ret = m_functions.Get(name); return std::make_pair(ret.first, ret.second); }
	...
	// TODO: Performance: When primitive type, first lookup in a static table, just loke in get_funcs.
	std::pair<MetaType*,size_t> get_meta_type(const std::string& name) {auto ret = m_meta_types.Get(name); return std::make_pair(ret.first, ret.second); }
